...
Зверніть увагу – назву предмету необхідно повертати з великої літери, а _ – замінити на пробіл</p>
<p style="margin:10px 0 0; padding:0; color:#097961">Результат виклику функції getSubjects(students[0]) : ${JSON.stringify(getSubjects(students[0]))} </p><br>`)
...
Можна використовувати функції, написані у попередніх домашніх завданнях :)</p>
<p style="margin:10px 0 0; padding:0; color:#097961">Результат виклику функції getAverageMark(students[0]) : ${getAverageMark(students[0])}</p><br>`);
...
студенту (вам знадобиться функція з попереднього завдання). Повинна бути виведена інформація: курс, ім'я, середня оцінка.</p> 
<p style="margin:10px 0 0; padding:0; color:#097961">Результат виклику функції getStudentInfo() : ${getStudentInfo(students[0])}</p><br>`);
...
document.writeln(`<p style="margin:0; padding:0">4. Ствроіть функцію getStudentsNames(students) --> ["Anton", "Tanya, "Victor"] – яка повертає імена студентів у алфавітному порядку</p>
<p style="margin:10px 0 0; padding:0; color: #097961">Результат виклику функції getStudentsNames(): ${JSON.stringify(getStudentsNames(students))} - відсортований масив імен студентів у алфавітному порядку</p><br>`);
...
document.writeln(`<p style="margin:0; padding:0">5. Створіть функцію getBestStudent(students) --> "Anton" – яка повертає кращого студента зі списку по показнику середньої оцінки.</p>
<p style="margin:10px 0 0; padding:0; color: #097961">Результат виклику функції getBestStudent(students): ${JSON.stringify(getBestStudent(students))}</p><br>`);
/*
...
document.writeln(`<p style="margin:0; padding:0">6. Створіть функцію calculateWordLetters("тест") --> { "т": 2, "е": 1, "с": 1 } – яка повертає обє'кт, в якому ключі це букви у слові, а значення – кількість їх повторень.</p>
 <p style="margin:10px 0 0; padding:0; color: #097961">Результат виклику функції calculateWordLetters("тест") : ${calculateWordLetters(str)}</p>`);
